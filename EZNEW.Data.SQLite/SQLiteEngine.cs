...
{
    /// <summary>
    /// Imeplements database engine for the SQLite
    /// </summary>
    public class SQLiteEngine : IDatabaseEngine
    {
        ...
        public async Task<int> ExecuteAsync(DatabaseServer server, CommandExecuteOptions executeOption, IEnumerable<ICommand> commands)
        {
            ...
            foreach (var cmd in commands)
            {
                ...
                //Trace log
                SQLiteFactory.LogExecuteCommand(executeCommand);
        ...
        public async Task<IEnumerable<T>> QueryAsync<T>(DatabaseServer server, ICommand command)
        {
            ...
            using (var conn = SQLiteFactory.GetConnection(server))
            {
                ...
                var cmdDefinition = new CommandDefinition(cmdText, parameters, transaction: tran, commandType: SQLiteFactory.GetCommandType(command as RdbCommand), cancellationToken: command.Query?.GetCancellationToken() ?? default);
        ...
        public async Task<DataSet> QueryMultipleAsync(DatabaseServer server, ICommand command)
        {
            ...
            using (var conn = SQLiteFactory.GetConnection(server))
            {
                ...
                var cmdDefinition = new CommandDefinition(command.CommandText, parameters, transaction: tran, commandType: SQLiteFactory.GetCommandType(command as RdbCommand), cancellationToken: command.Query?.GetCancellationToken() ?? default);

...
namespace EZNEW.Data.SQLite
{
    ...
    internal static class SQLiteFactory
    {
        ...
        /// Field format key
        /// </summary>
        internal static readonly string FieldFormatKey = ((int)DatabaseServerType.SQLite).ToString();
        ...
        {
            return DataManager.GetDatabaseConnection(server) ?? new SqliteConnection(server.ConnectionString);
        }
        ...
        {
            return DataManager.GetQueryTranslator(server.ServerType) ?? new SQLiteQueryTranslator();
        }
        ...
        internal static string ParseCriteriaConverter(ICriteriaConverter converter, string objectName, string fieldName)
        {
            ...
            return criteriaConverterParse(new CriteriaConverterParseOptions()
            {
                ...
                ServerType = DatabaseServerType.SQLite,
        ...
        static string Parse(CriteriaConverterParseOptions option)
        {
            ...
            switch (option.CriteriaConverter.Name)
            {
                case CriteriaConverterNames.StringLength:
                    format = $"LENGTH({option.ObjectName}.{WrapKeyword(option.FieldName)})";
                    break;
            }
            if (string.IsNullOrWhiteSpace(format))
            {
                throw new EZNEWException($"Cann't resolve criteria convert:{option.CriteriaConverter.Name} for SQLite");
            }
            return format;
        }
        ...
        {
            return DataManager.GetQueryFields(DatabaseServerType.SQLite, entityType, query, forceMustFields);
        }
        ...
        {
            return DataManager.GetFields(DatabaseServerType.SQLite, entityType, propertyNames);
        }
        ...
        public static string GetDefaultFieldName(Type entityType)
        {
            ...
            return DataManager.GetDefaultField(DatabaseServerType.SQLite, entityType)?.FieldName ?? string.Empty;
        }
        ...
        public static IsolationLevel? GetTransactionIsolationLevel(DataIsolationLevel? dataIsolationLevel)
        {
            ...
            {
                dataIsolationLevel = DataManager.GetDataIsolationLevel(DatabaseServerType.SQLite);
            }

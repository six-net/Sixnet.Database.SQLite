...
{
    /// <summary>
    /// Query translator implement for the SQLite
    /// </summary>
    public class SQLiteQueryTranslator : IQueryTranslator
    {
            ...
            if (query.QueryType == QueryCommandType.QueryObject)
            {
                ...
                if (!query.CombineItems.IsNullOrEmpty())
                {
                    foreach (var combine in query.CombineItems)
                    {
                        ...
                        string combineObjectName = DataManager.GetQueryRelationObjectName(DatabaseServerType.SQLite, combine.CombineQuery);
                        ...
                        combineBuilder.Append($" {GetCombineOperator(combine.CombineType)} SELECT {string.Join(",", SQLiteFactory.FormatQueryFields(combineObjectPetName, query, query.GetEntityType(), true, false))} FROM {SQLiteFactory.WrapKeyword(combineObjectName)} AS {combineObjectPetName} {(combineQueryResult.AllowJoin ? combineQueryResult.JoinScript : string.Empty)} {combineConditionString}");
                ...
                if (!query.JoinItems.IsNullOrEmpty())
                {
                    foreach (var joinItem in query.JoinItems)
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(joinQueryResult.CombineScript))
                        {
                            ...
                            joinBuilder.Append($" {GetJoinOperator(joinItem.JoinType)} {SQLiteFactory.WrapKeyword(DataManager.GetQueryRelationObjectName(DatabaseServerType.SQLite, joinItem.JoinQuery))} AS {joinObjName}{joinConnection}");
                        ...
                        else
                        {
                            ...
                            joinBuilder.Append($" {GetJoinOperator(joinItem.JoinType)} (SELECT {string.Join(",", SQLiteFactory.FormatQueryFields(joinObjName, joinItem.JoinQuery, joinItem.JoinQuery.GetEntityType(), false, false))} FROM {SQLiteFactory.WrapKeyword(DataManager.GetQueryRelationObjectName(DatabaseServerType.SQLite, joinItem.JoinQuery))} AS {joinObjName} {(joinQueryResult.AllowJoin ? joinQueryResult.JoinScript : string.Empty)} {(string.IsNullOrWhiteSpace(joinQueryResult.ConditionString) ? string.Empty : "WHERE " + joinQueryResult.ConditionString)} {joinQueryResult.CombineScript}) AS {combineJoinObjName}{joinConnection}");
                        }
                ...
                if (query.RecurveCriteria != null)
                {
                    ...
                    EntityField recurveField = DataManager.GetField(DatabaseServerType.SQLite, query, query.RecurveCriteria.Key);
                    EntityField recurveRelationField = DataManager.GetField(DatabaseServerType.SQLite, query, query.RecurveCriteria.RelationKey);
                    var recurveTable = GetNewRecurveTableName();
                    recurveTablePetName = recurveTable.Item1;
                    recurveTableName = recurveTable.Item2;
                    conditionString = $"{objectName}.{SQLiteFactory.WrapKeyword(recurveField.FieldName)} IN (SELECT {recurveTablePetName}.{SQLiteFactory.WrapKeyword(recurveField.FieldName)} FROM {SQLiteFactory.WrapKeyword(recurveTableName)} AS {recurveTablePetName})";
                    string queryObjectName = DataManager.GetQueryRelationObjectName(DatabaseServerType.SQLite, query);
                    string withScript =
                    $"{recurveTableName} AS (SELECT {objectName}.{SQLiteFactory.WrapKeyword(recurveField.FieldName)},{objectName}.{SQLiteFactory.WrapKeyword(recurveRelationField.FieldName)} FROM {SQLiteFactory.WrapKeyword(queryObjectName)} AS {objectName} {joinScript} {(string.IsNullOrWhiteSpace(nowConditionString) ? string.Empty : $"WHERE {nowConditionString}")} " +
                    $"UNION ALL SELECT {objectName}.{SQLiteFactory.WrapKeyword(recurveField.FieldName)},{objectName}.{SQLiteFactory.WrapKeyword(recurveRelationField.FieldName)} FROM {SQLiteFactory.WrapKeyword(queryObjectName)} AS {objectName} JOIN {recurveTableName} AS {recurveTablePetName} " +
                    $"ON {(query.RecurveCriteria.Direction == RecurveDirection.Up ? $"{objectName}.{SQLiteFactory.WrapKeyword(recurveField.FieldName)}={recurveTablePetName}.{SQLiteFactory.WrapKeyword(recurveRelationField.FieldName)}" : $"{objectName}.{SQLiteFactory.WrapKeyword(recurveRelationField.FieldName)}={recurveTablePetName}.{SQLiteFactory.WrapKeyword(recurveField.FieldName)}")})";
        ...
        TranslateResult TranslateCriteria(IQuery query, Criteria criteria, CommandParameters parameters, string objectName)
        {
            ...
            if (valueQuery != null)
            {
                var valueQueryObjectName = DataManager.GetQueryRelationObjectName(DatabaseServerType.SQLite, valueQuery);
                ...
                var valueQueryField = DataManager.GetField(DatabaseServerType.SQLite, valueQuery, valueQuery.QueryFields.First());
                ...
                if (subqueryLimitResult.Item1)
                {
                    valueQueryCondition = string.IsNullOrWhiteSpace(subQueryResult.CombineScript)
                        ? $"{criteriaFieldName} {sqlOperator} (SELECT {SQLiteFactory.WrapKeyword(valueQueryField.FieldName)} FROM (SELECT {subObjName}.{SQLiteFactory.WrapKeyword(valueQueryField.FieldName)} FROM {SQLiteFactory.WrapKeyword(valueQueryObjectName)} AS {subObjName} {(subQueryResult.AllowJoin ? subQueryResult.JoinScript : string.Empty)} {conditionString} {orderString} {topString}) AS S{subObjName})"
                        : $"{criteriaFieldName} {sqlOperator} (SELECT {SQLiteFactory.WrapKeyword(valueQueryField.FieldName)} FROM (SELECT {subObjName}.{SQLiteFactory.WrapKeyword(valueQueryField.FieldName)} FROM (SELECT {string.Join(",", SQLiteFactory.FormatQueryFields(subObjName, valueQuery, valueQuery.GetEntityType(), true, false))} FROM {SQLiteFactory.WrapKeyword(valueQueryObjectName)} AS {subObjName} {(subQueryResult.AllowJoin ? subQueryResult.JoinScript : string.Empty)} {conditionString} {subQueryResult.CombineScript}) AS {subObjName} {orderString} {topString}) AS S{subObjName})";
                }
                else
                {
                    valueQueryCondition = string.IsNullOrWhiteSpace(subQueryResult.CombineScript)
                        ? $"{criteriaFieldName} {sqlOperator} (SELECT {subObjName}.{SQLiteFactory.WrapKeyword(valueQueryField.FieldName)} FROM {SQLiteFactory.WrapKeyword(valueQueryObjectName)} AS {subObjName} {(subQueryResult.AllowJoin ? subQueryResult.JoinScript : string.Empty)} {conditionString} {orderString} {topString})"
                        : $"{criteriaFieldName} {sqlOperator} (SELECT {subObjName}.{SQLiteFactory.WrapKeyword(valueQueryField.FieldName)} FROM (SELECT {string.Join(",", SQLiteFactory.FormatQueryFields(subObjName, valueQuery, valueQuery.GetEntityType(), true, false))} FROM {SQLiteFactory.WrapKeyword(valueQueryObjectName)} AS {subObjName} {(subQueryResult.AllowJoin ? subQueryResult.JoinScript : string.Empty)} {conditionString} {subQueryResult.CombineScript}) AS {subObjName} {orderString} {topString})";
                }
            ...
            var needWrapParameter = NeedWrapParameter(criteria.Operator);
            if (needWrapParameter)
            {
                var parameterValues = criteria.GetCriteriaRealValue();
                List<string> parameterStringValues = new List<string>();
                foreach (var val in parameterValues)
                {
                    parameterStringValues.Add($"'{val?.ToString() ?? string.Empty}'");
                }
                var criteriaCondition = $"{criteriaFieldName} {sqlOperator} ({string.Join(",", parameterStringValues)})";
            ...
            else
            {
                ...
                var criteriaCondition = $"{criteriaFieldName} {sqlOperator} {SQLiteFactory.ParameterPrefix}{parameterName}";
                return TranslateResult.CreateNewResult(criteriaCondition);
            }
        }
        ...
        {
            var field = DataManager.GetField(DatabaseServerType.SQLite, query, fieldName);
            fieldName = field.FieldName;
            if (convert == null)
            {
                return $"{objectName}.{SQLiteFactory.WrapKeyword(fieldName)}";
            }
            return SQLiteFactory.ParseCriteriaConverter(convert, objectName, fieldName);
        }
        ...
        string GetJoinCondition(IQuery sourceQuery, JoinItem joinItem, string sourceObjShortName, string targetObjShortName)
        {
            ...
            foreach (var joinField in joinFields)
            {
                ...
                var sourceField = DataManager.GetField(DatabaseServerType.SQLite, sourceEntityType, joinField.Key);
                var targetField = DataManager.GetField(DatabaseServerType.SQLite, targetEntityType, joinField.Value);
                joinList.Add($" {sourceObjShortName}.{SQLiteFactory.WrapKeyword(useValueAsSource ? targetField.FieldName : sourceField.FieldName)}{GetJoinOperator(joinItem.Operator)}{targetObjShortName}.{SQLiteFactory.WrapKeyword(useValueAsSource ? sourceField.FieldName : targetField.FieldName)}");
            }
        ...
        bool NeedWrapParameter(CriteriaOperator criteriaOperator)
        {
            switch (criteriaOperator)
            {
                case CriteriaOperator.In:
